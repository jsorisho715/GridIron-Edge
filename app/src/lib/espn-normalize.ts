...
export function normalizeLeague(core:unknown, cards:unknown, free:unknown, schedule:unknown, input:ESPNInput, now=Date.now()):Snapshot {
  ...
  for(const [key,value] of Object.entries(counts)){const count=Math.max(0,Math.min(30,num(value))),id=Number(key);rosterLimit+=count;if([20,21,24].includes(id)||!count)continue;for(let i=0;i<count;i++)slots.push({id:key+':'+i,espnId:id,label:(SLOT_NAMES[id]??'Slot '+id)+(count>1?' '+(i+1):'')});}
  ...
  const normalize=(entry:Raw,teamId:number|null,slot:string|null,slotId:number|null):LeaguePlayer|null=>{
    ...
    const card=cardMap.get(id),p={...base,...card},statRows=arr(card?.stats??base.stats),history:GameLog[]=[];
    ...
    return {id,name:name(p.fullName,'Player '+id),position:POS[num(p.defaultPositionId)]??SLOT_NAMES[eligible.find((s:number)=>![20,21,23,24,25].includes(s))]??'OTHER',proTeam:PRO[proTeamId]??'NFL',proTeamId,teamId,slot,slotId,eligible,
      ...
      locked:!scheduleKnown||(!bye&&date!==null&&date<=now),kickoff,opponent:game?PRO[game.homeProTeamId===proTeamId?game.awayProTeamId:game.homeProTeamId]??null:null,bye,scheduleKnown,
