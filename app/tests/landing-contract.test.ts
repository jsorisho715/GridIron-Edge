...
describe("private decision-desk website contract", () => {
  ...
  test("opens the requested decision desk directly, preserving the template engine", () => {
    ...
    // The owner explicitly chose direct dashboard entry, without an intro film.
    expect(landingRoute).toContain("component:Workspace");
    expect(appRoute).toContain("component:Workspace");
    expect(appRoute).toContain("createFileRoute('/app')");
    expect(readFileSync(new URL("../src/routes/connections.tsx", import.meta.url), "utf8")).toContain('createFileRoute("/connections")');
  });
